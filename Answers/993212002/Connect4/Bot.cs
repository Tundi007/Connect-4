...
public class MyBot
{
    ...
    private static int PlayerMaxPoints_Function(Matrix<Single> appliedBoard_SingleMatrix)
    {
        ...
        for (int nextMove_Int = 0; nextMove_Int < 5; nextMove_Int++)
        {
            ...
            MyMatrix.ApplyMove_Function(copyBoard_SingleMatrix, nextMove_Int, BID_Int).CopyTo(copyBoard_SingleMatrix);
    ...
    public static (int, int) MaxPiece_Function(Matrix<Single> appliedBoard_SingleMatrix)
    {
        ...
        for (int corner_Int = 0; corner_Int < 4; corner_Int++)
        {
            ...
            MyMatrix.FourByFour_Function(appliedBoard_SingleMatrix, corner_Int).CopyTo(fourByFour_SingleMatrix);
    ...
    private static (int, int) IDCheckVector_Function(Vector<Single> vector_SingleVector, int botMaxPieces_Int, int playerMaxPieces_Int)
    {
        ...
        while (vector_SingleVector.Contains(PID_Int) | vector_SingleVector.Contains(BID_Int))
        {
            ...
            if(MyMatrix.CountID_Function(vector_SingleVector, 0) != 0)
            for (int index_Int = 0; index_Int < MyMatrix.CountID_Function(vector_SingleVector, 0); index_Int++)
            {
            ...
            if(MyMatrix.CountID_Function(vector_SingleVector, BID_Int) != 0)
            {

                botMaxPieces_Int = Compare_Function(botMaxPieces_Int, MyMatrix.CountID_Function(vector_SingleVector, BID_Int));

                for (int index_Int = 0; index_Int < MyMatrix.CountID_Function(vector_SingleVector, BID_Int); index_Int++)
                {
            ...
            if(MyMatrix.CountID_Function(vector_SingleVector, PID_Int) != 0)
            {

                botMaxPieces_Int = Compare_Function(botMaxPieces_Int, MyMatrix.CountID_Function(vector_SingleVector, PID_Int));

                for (int index_Int = 0; index_Int < MyMatrix.CountID_Function(vector_SingleVector, PID_Int); index_Int++)
                {
